...
var hideAlert = function hideAlert() {
  ...
  if (el) el.parentElement.removeChild(el); //confused about this? google the documentation
...
var showAlert = function showAlert(type, msg) {
  ...
  window.setTimeout(hideAlert, 5000); //this means place inside the body but right at the beginning
        ...
        switch (_context.prev = _context.next) {
          case 0:
            ...
            return (0, _axios.default)({
              ...
              //can do this because the api is on the same url, additional path added to current url like we did with image upload
              // url: 'http://127.0.0.1:3000/api/v1/users/login', can;t use this in production
              data: {
                email: email,
                //   email:email, eslint you just put email
                password: password
              }
            });
...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiY2tvcm5oaXNlciIsImEiOiJja2k4MjRlbGIwMWpwMnNzNTZzNXhiN2UzIn0.wbQ2Zx6uBKNsVksB6cGzQQ';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/ckornhiser/cki82lac500dq18mwznxhxv1k',
    scrollZoom: false //   center: [-118.243683, 34.052235], //long then lat
    //   zoom: 10,
    //   interactive: false,

  });
  var bounds = new mapboxgl.LngLatBounds(); //The area that will be show on the map
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? '/api/v1/users/updateMyPassword' : '/api/v1/users/updateMe'; // ? 'http://127.0.01:3000/api/v1/users/updateMyPassword'
...
var stripe = Stripe('pk_test_51HvVJZH4OqSHUwdZF7FNqpWVSuG9zjBMssuRV6w4bY5elIsn1bvqMXVswWScZlJPeobfvI4Gl0T8dHgR1nIXV7MM00Urwq1wx2'); //this object is gloabaly available from tour.pug script tag
...
//allows new JS to be seen on old browsers
//DOM ELEMENTS
...
var bookBtn = document.getElementById('book-tour'); // DELEGATION --makes sure map is trying to load on every view
...
if (userDataForm) userDataForm.addEventListener('submit', function (e) {
  e.preventDefault(); //create multiform datta form

  var form = new FormData();
  form.append('name', document.getElementById('name').value);
  form.append('email', document.getElementById('email').value);
  form.append('photo', document.getElementById('photo').files[0]); //since files in an array and there is only one we take the first\
...
if (userPasswordForm) userPasswordForm.addEventListener('submit', function (e) {
  e.preventDefault();
  document.querySelector('.btn--save-password').textContent = 'Updating...'; //these are the names of the values that our api expcects to see
  ...
  }, 'password'); //sent to api, matches if you did this manually in postman
...
if (bookBtn) bookBtn.addEventListener('click', function (e) {
  ...
  (0, _stripe.bookTour)(tourId); //this id is passed to this function located in stripe.js
});
